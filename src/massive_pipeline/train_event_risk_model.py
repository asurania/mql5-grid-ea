...
IN_FILE = Path("data/processed/modeling/event_risk_training_dataset.parquet")
OUT_DIR = Path("data/models/event_risk")
...
def main() -> None:
    ...
    pdf = df.to_pandas()
    pdf = pdf.sort_values(["event_timestamp_utc", "pair"]).reset_index(drop=True)
    ...
    model.save_model(str(OUT_DIR / "xgb_y_post_news_session_risk.json"))
