...
OUT_DIR = Path("data/models/avoid_session_lgbm")
...
def main() -> None:
    ...
    model = lgb.LGBMClassifier(
        ...
        verbose=-1,
        categorical_feature=[feature_cols.index(c) for c in CATEGORICAL_COLUMNS if c in feature_cols],
    )
    ...
    import joblib
    joblib.dump(model, str(OUT_DIR / "lgbm_y_avoid_session.joblib"))
